...
export default function monitorNetwork(showNetworkRequests: any, showNetworkResponses: any) {
  const requestCache = {};
  ...
  if (showNetworkRequests) {
    XHRInterceptor.setSendCallback((data: string, xhr: { _url: string; _trackingName: number; _method: any; }) => {
  ...
  if (showNetworkResponses) {
    XHRInterceptor.setResponseCallback((status: number, timeout: any, response: string, url: string, type: any, xhr: { _trackingName: any; _method: any; }) => {
